        ...
        for i in range(0, len(prompts), batch_size):
            ...
            for prompt in batch:
                try:
                    ...
                    batch_results.append("0")  # Default to Non-Functional on error
...
"1": Functional (e.g., defines specific system functions, behaviors, or operations)
"0": Non-Functional (e.g., specifies performance, usability, reliability, or constraints)
...
def compute_classification_metrics(y_true, y_pred):
    report = classification_report(y_true, y_pred, target_names=['Non-Functional', 'Functional'], output_dict=True, zero_division=0)
    metrics = {
        'Functional': {
            'precision': report['Functional']['precision'],
            'recall': report['Functional']['recall'],
            'f1': report['Functional']['f1-score'],
            'f2': fbeta_score(y_true, y_pred, beta=2, pos_label=1, zero_division=0)
        },
        'Non-Functional': {
            'precision': report['Non-Functional']['precision'],
            'recall': report['Non-Functional']['recall'],
            'f1': report['Non-Functional']['f1-score'],
...
def majority_vote(predictions):
    ...
    for i in range(len(predictions[0])):
        ...
        if max(counts) >= len(predictions) // 2 + 1:
            ...
            voted.append('0')  # Default to Non-Functional
    return voted
...
def prepare_data(df):
    ...
    functional_examples = df_train.drop_duplicates()[['text', 'label']]
    return functional_examples, df_train, df_test
...
def generate_summary_table_custom(all_metrics, num_runs):
    ...
    with open('all_summary_metrics_nfr.csv', 'w') as f:
        for llm in all_data:
            f.write(f"{llm}\n")
            f.write("\tAverage\t\t\t\tFunctional\t\tNon-Functional\n")
            ...
            for method in methods:
                ...
                f.write(f"{method_name}\t{avg['P']:.3f}\t{avg['R']:.3f}\t{avg['F1']:.3f}\t{avg['F2']:.3f}\t"
                        f"{best['Functional']['F1']:.3f}\t{best['Functional']['F2']:.3f}\t"
                        f"{best['Non-Functional']['F1']:.3f}\t{best['Non-Functional']['F2']:.3f}\n")
            f.write("\n\n")

    return pd.DataFrame()

def run_api_classification(api_client, prompts_list, labels, df_test, method_name, run_num):
    """Run classification using API clients for Functional/Non-Functional tasks"""
...
def run_classification(df_uploaded, num_runs=3, base_batch_size=16):
    ...
    logger.info(f"Ground truth label counts:\n{df_uploaded.groupby(['label']).size().to_string()}")
    ...
    for llm in llms:
        ...
        for run in range(num_runs):
            ...
            functional_examples, df_train, df_test = prepare_data(df_uploaded)
            ...
            for method, prompts in methods_f.items():
                dataset_f = Dataset.from_dict({"prompt": prompts})
                predictions = []
                num_voting_runs = 3
                start_time = time.time()
                for _ in range(num_voting_runs):
                    outputs_f = pipeline(dataset_f["prompt"], max_new_tokens=1024, eos_token_id=terminators[0], batch_size=batch_size)
                    preds_f = mapping(remove_assistant_prefix(outputs_f), labels_functional)
                    predictions.append(preds_f)
                    if len(predictions) >= 2 and check_prediction_variance(predictions):
                        break
                end_time = time.time()
                running_time = end_time - start_time
                ...
                voted_preds = majority_vote(predictions)
                metrics = compute_classification_metrics(df_test['label'].map({'1': 1, '0': 0}), [int(p) for p in voted_preds])
        ...
        for run in range(num_runs):
            ...
            for method, prompts in methods_f.items():
                metrics, running_time = run_api_classification(
                    api_client, prompts, labels_functional, df_test, method, run
                )
    ...
    timing_df.to_csv('llm_task_timing_nfr.csv', index=False)
    logger.info("Timing data saved to 'llm_task_timing_nfr.csv'.")

    generate_summary_table_custom(all_llm_results, num_runs)
    logger.info("Summary metrics saved to 'all_summary_metrics_nfr.csv'.")
...
if __name__ == "__main__":
    ...
    df_uploaded['label'] = df_uploaded['Class'].apply(lambda x: '1' if x == 'F' else '0')
